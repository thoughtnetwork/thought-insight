...
/**
 * Will remove a service from dashcore-node.json
...
/**
 * Will remove the Node.js service and from the dashcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The dashcore-node configuration directory
...
function remove(options, done) {
  ...
  var dashcoreConfigPath = path.resolve(configPath, 'dashcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(dashcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a dashcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to dashcore-node.json
        removeConfig(dashcoreConfigPath, service, next);
      });
    }, done
  );
}
