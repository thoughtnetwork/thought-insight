/*! @dashevo/insight-ui 2.0.0 */

var defaultLanguage = localStorage.getItem("insight-language") || "en",
    defaultCurrency = localStorage.getItem("insight-currency") || "THT";
angular.module("insight", ["ngAnimate", "ngResource", "ngRoute", "ngProgress", "ui.bootstrap", "ui.route", "monospaced.qrcode", "gettext", "angularMoment", "insight.system", "insight.socket", "insight.blocks", "insight.transactions", "insight.address", "insight.search", "insight.status", "insight.connection", "insight.currency", "insight.messages"]), angular.module("insight.system", []), angular.module("insight.socket", []), angular.module("insight.blocks", []), angular.module("insight.transactions", []), angular.module("insight.address", []), angular.module("insight.search", []), angular.module("insight.status", []), angular.module("insight.connection", []), angular.module("insight.currency", []), angular.module("insight.messages", []), angular.module("insight.address").controller("AddressController", function ($scope, $rootScope, $routeParams, $location, Global, Address, getSocket) {
    $scope.global = Global;
    var socket = getSocket($scope), addrStr = $routeParams.addrStr, _startSocket = function () {
        socket.on("dashd/addresstxid", function (data) {
            if (data.address === addrStr) {
                $rootScope.$broadcast("tx", data.txid);
                var base = document.querySelector("base");
                new Audio(base.href + "/sound/transaction.mp3").play()
            }
    ...
    socket.on("connect", function () {
        ...
        $rootScope.currentAddr = $routeParams.addrStr, _startSocket(), Address.get({addrStr: $routeParams.addrStr}, function (address) {
    ...
    $scope.global = Global, $scope.loading = !1, $routeParams.blockHeight && BlockByHeight.get({blockHeight: $routeParams.blockHeight}, function (hash) {
    ...
    $scope.toUTCDate = toUTCDate, $scope.millisToUTCDate = function (millis) {
        return toUTCDate(new Date(1e3 * millis))
    };
    $scope.$watch("dt", function (newValue, oldValue) {
        var date, yyyy, mm, dd;
        newValue !== oldValue && $location.path("/blocks-date/" + (yyyy = (date = newValue).getUTCFullYear().toString(), mm = (date.getUTCMonth() + 1).toString(), dd = date.getUTCDate().toString(), yyyy + "-" + (mm[1] ? mm : "0" + mm[0]) + "-" + (dd[1] ? dd : "0" + dd[0])))
    }), $scope.openCalendar = function ($event) {
        $scope.opened = !0, setTimeout(function () {
            angular.element(document.querySelector(".uib-datepicker-popup"))[0].style.display = "block"
        }, 100)
    }, $scope.humanSince = function (time) {
        var m = moment.unix(time).startOf("day"), b = moment().startOf("day");
        return m.max().from(b)
    }, $scope.list = function () {
        if ($scope.loading = !0, $routeParams.blockDate && ($scope.detail = "On " + $routeParams.blockDate), $routeParams.startTimestamp) {
            var d = new Date(1e3 * $routeParams.startTimestamp), m = d.getMinutes();
            m < 10 && (m = "0" + m), $scope.before = " before " + d.getHours() + ":" + m
        }
        $rootScope.titleDetail = $scope.detail, Blocks.get({
            blockDate: $routeParams.blockDate,
            startTimestamp: $routeParams.startTimestamp
        }, function (res) {
            for (var key in $scope.loading = !1, $scope.blocks = res.blocks, $scope.pagination = res.pagination, $scope.pagination.olderBlocks = $scope.pagination.moreTs, $scope.blocks) $scope.blocks[key].time < $scope.pagination.olderBlocks && ($scope.pagination.olderBlocks = $scope.blocks[key].time)
        })
    }, $scope.findOne = function () {
        $scope.loading = !0, Block.get({blockHash: $routeParams.blockHash}, function (block) {
...
}), angular.module("insight.currency", ["ui.router", "ui.bootstrap"]).controller("CurrencyController", function ($scope, $rootScope, Currency) {
    ...
    $rootScope.currency.getConvertion = function (value) {
        var response;
        return value *= 1, isNaN(value) || null == value ? "value error" : 0 === value ? "0 " + this.symbol : ((response = "USD" === this.symbol ? _roundFloat(value * this.factor, 2) : "mTHT" === this.symbol ? (this.factor = 1e3, _roundFloat(value * this.factor, 5)) : "uTHT" === this.symbol ? (this.factor = 1e6, _roundFloat(value * this.factor, 2)) : (this.factor = 1, value)) < 1e-7 && (response = response.toFixed(8)), response + " " + this.symbol)
        ...
        }) : $rootScope.currency.factor = "mTHT" === currency ? 1e3 : "uTHT" === currency ? 1e6 : 1
    ...
    $scope.version = void Version.get({}, function (res) {
        $scope.version = res.version
    }), $scope.availableLanguages = [{name: "Deutsch", isoCode: "de_DE"}, {
        name: "English",
        isoCode: "en"
    }, {name: "French", isoCode: "fr_FR"}, {name: "Spanish", isoCode: "es"}, {
        name: "Portuguese",
        isoCode: "pt_BR"
    }, {name: "Japanese", isoCode: "ja"}, {name: "Chinese", isoCode: "zh"}], $scope.setLanguage = function (isoCode) {
...
}), angular.module("insight.system").controller("HeaderController", function ($scope, $rootScope, $uibModal, getSocket, Global, Block) {
    $scope.global = Global, $rootScope.currency = {
        factor: 1,
        bitstamp: 0,
        symbol: "THT"
    }, $scope.menu = [{title: "Blocks", link: "blocks"}, {
        title: "Status",
        link: "status"
    }], $scope.openScannerModal = function () {
        $uibModal.open({templateUrl: "scannerModal.html", controller: "ScannerController"})
    };
    var socket = getSocket($scope);
    socket.on("connect", function () {
        socket.emit("subscribe", "inv"), socket.on("block", function (block) {
            var hash, blockHash = block.toString();
            hash = blockHash, Block.get({blockHash: hash}, function (res) {
                $scope.totalBlocks = res.height
            })
        })
    }), $rootScope.isCollapsed = !0
});
var TRANSACTION_DISPLAYED = 10, BLOCKS_DISPLAYED = 5;
angular.module("insight.system").controller("IndexController", function ($scope, Global, getSocket, Blocks) {
    $scope.global = Global;
    var _getBlocks = function () {
        Blocks.get({limit: BLOCKS_DISPLAYED}, function (res) {
            $scope.blocks = res.blocks, $scope.blocksLength = res.length
        })
    }, socket = getSocket($scope), _startSocket = function () {
    ...
    $scope.message = {address: "", signature: "", message: ""}, $scope.verification = {
        status: "unverified",
        result: null,
        error: null,
        address: ""
        ...
        $scope.verification.status = "loading", $scope.verification.address = $scope.message.address, $http.post(window.apiPrefix + "/messages/verify", $scope.message).success(function (data, status, headers, config) {
            if ("boolean" != typeof data.result) return $scope.verification.status = "error", void ($scope.verification.error = null);
            $scope.verification.status = "verified", $scope.verification.result = data.result
...
}), angular.module("insight.system").controller("ScannerController", function ($scope, $rootScope, $uibModalInstance, Global) {
    $scope.global = Global;
    var isMobile = {
        Android: function () {
    ...
    var video, canvas, context, localMediaStream, $searchInput = angular.element(document.getElementById("search")),
        _scan = function (evt) {
            if ($scope.isMobile) {
                ...
                if (1 === files.length && 0 === files[0].type.indexOf("image/")) {
                    ...
                    reader.onload = function (e) {
                        new MegaPixImage(file).render(canvas, {
                            maxWidth: 200,
                            maxHeight: 200,
                            orientation: 6
                        }), setTimeout(function () {
            ...
            $scope.scannerLoading = !1, $uibModalInstance.close(), $scope.isMobile || (localMediaStream.stop && localMediaStream.stop(), localMediaStream = null, video.src = "")
        }, _videoError = function (err) {
            console.log("Video Error: " + JSON.stringify(err)), _scanStop()
        };
    qrcode.callback = function (data) {
        _scanStop();
        var str = 0 === data.indexOf("dash:") ? data.substring(8) : data;
    ...
    $scope.search = function () {
        var q = $scope.q;
        $scope.badQuery = !1, $scope.loading = !0, console.log("xx2x"), Block.get({blockHash: q}, function (res) {
            console.log("block status: ", res);
            _resetSearch(), $location.path("block/" + q)
        }, function(res) {
            console.log("block error status: ", res);
            if (res.status === 404) {
                Transaction.get({txId: q}, function (res) {
                    console.log("transaction status: ", res);
                    _resetSearch(), $location.path("tx/" + q);
                }, function (res) {
                    console.log("transaction error status: ", res);
                    if (res.status === 404 || res.status === 400) {
                        Address.get({addrStr: q}, function (res) {
                            console.log("address status: ", res);
                            _resetSearch(), $location.path("address/" + q)
                        }, function (res) {
                            console.log("address error status: " + res);
                            if (res.status === 404) {
                            isFinite(q) ? BlockByHeight.get({blockHeight: q}, function (hash) {
    ...
    $scope.global = Global, $scope.getStatus = function (q) {
        Status.get({q: "get" + q}, function (d) {
            $scope.loaded = 1, angular.extend($scope, d)
        }, function (e) {
            $scope.error = "API ERROR: " + e.data
        })
    }, $scope.humanSince = function (time) {
        return moment.unix(time / 1e3).max().fromNow()
    };
    ...
    var pageNum = 0, pagesTotal = 1, _aggregateItems = function (items) {
        if (!items) return [];
        for (var l = items.length, ret = [], tmp = {}, u = 0, i = 0; i < l; i++) {
            var notAddr = !1;
            if (items[i].scriptSig && !items[i].addr && (items[i].addr = "Unparsed address [" + u++ + "]", notAddr = items[i].notAddr = !0), items[i].scriptPubKey && !items[i].scriptPubKey.addresses && (items[i].scriptPubKey.addresses = ["Unparsed address [" + u++ + "]"], notAddr = items[i].notAddr = !0), items[i].scriptPubKey && 1 < items[i].scriptPubKey.addresses.length) items[i].addr = items[i].scriptPubKey.addresses.join(","), ret.push(items[i]); else {
                var addr = items[i].addr || items[i].scriptPubKey && items[i].scriptPubKey.addresses[0];
                tmp[addr] || (tmp[addr] = {}, tmp[addr].valueSat = 0, tmp[addr].count = 0, tmp[addr].addr = addr, tmp[addr].items = []), tmp[addr].isSpent = items[i].spentTxId, tmp[addr].doubleSpentTxID = tmp[addr].doubleSpentTxID || items[i].doubleSpentTxID, tmp[addr].doubleSpentIndex = tmp[addr].doubleSpentIndex || items[i].doubleSpentIndex, tmp[addr].dbError = tmp[addr].dbError || items[i].dbError, tmp[addr].valueSat += Math.round(1e8 * items[i].value), tmp[addr].items.push(items[i]), tmp[addr].notAddr = notAddr, items[i].unconfirmedInput && (tmp[addr].unconfirmedInput = !0), tmp[addr].count++
            }
        }
        return angular.forEach(tmp, function (v) {
            v.value = v.value || parseInt(v.valueSat) / 1e8, ret.push(v)
        }), ret
        ...
        Transaction.get({txId: txid}, function (tx) {
    ...
    $scope.findThis = function () {
        ...
        pageNum < pagesTotal && !$scope.loading && ($scope.loading = !0, "address" === $scope.loadedBy ? TransactionsByAddress.get({
            address: $routeParams.addrStr,
            pageNum: pageNum
        }, function (data) {
            _paginate(data)
        }) : TransactionsByBlock.get({block: $routeParams.blockHash, pageNum: pageNum}, function (data) {
            _paginate(data)
        }))
    }, ">" != $routeParams.v_type && "<" != $routeParams.v_type || ($scope.from_vin = "<" == $routeParams.v_type, $scope.from_vout = ">" == $routeParams.v_type, $scope.v_index = parseInt($routeParams.v_index), $scope.itemsExpanded = !0), $scope.txs = [], $scope.$on("tx", function (event, txid) {
        _findTx(txid)
    })
    ...
    $scope.transaction = "", $scope.status = "ready", $scope.txid = "", $scope.error = null, $scope.formValid = function () {
        ...
        $scope.status = "loading", $http.post(window.apiPrefix + "/tx/send", postData).success(function (data, status, headers, config) {
            if ("string" != typeof data.txid) return $scope.status = "error", void ($scope.error = "The transaction was sent but no transaction id was got back");
            $scope.status = "sent", $scope.txid = data.txid
        }).error(function (data, status, headers, config) {
            $scope.status = "error", $scope.error = data || "No error message given (connection error?)"
        })
    }
}), angular.module("insight.address").factory("Address", function ($resource) {
    return $resource(window.apiPrefix + "/addr/:addrStr/?noTxList=1", {addrStr: "@addStr"}, {
        get: {
            method: "GET",
            interceptor: {
                response: function (res) {
                    ...
                    if (404 === res.status) return res
                }*/
            }
        }
    })
}), angular.module("insight.blocks").factory("Block", function ($resource) {
    return $resource(window.apiPrefix + "/block/:blockHash", {blockHash: "@blockHash"}, {
        get: {
            method: "GET",
            interceptor: {
                response: function (res) {
                    ...
                    console.log("INTERCEPTOR RESPONSEERROR2: " + res.status);
//                    return res;
                    if (404 === res.status) return res
                }*/
            }
...
ScopedSocket.prototype.removeAllListeners = function (opts) {
    ...
    var socket = this.socket, $rootScope = this.$rootScope, wrapped_callback = function () {
        var args = arguments;
    ...
    var socket = this.socket, $rootScope = this.$rootScope, args = Array.prototype.slice.call(arguments);
    args.push(function () {
        var args = arguments;
        $rootScope.$apply(function () {
    ...
    var socket = io.connect(null, {reconnect: !0, "reconnection delay": 500});
    return function (scope) {
        var scopedSocket = new ScopedSocket(socket, $rootScope);
        return scope.$on("$destroy", function () {
            scopedSocket.removeAllListeners()
        }), socket.on("connect", function () {
            scopedSocket.removeAllListeners({skipConnect: !0})
        }), scopedSocket
    }
}), angular.module("insight.status").factory("Status", function ($resource) {
    return $resource(window.apiPrefix + "/status", {q: "@q"})
    ...
    return $resource(window.apiPrefix + "/tx/:txId", {txId: "@txId"}, {
        get: {
            method: "GET",
            interceptor: {
                response: function (res) {
    ...
    return $resource(window.apiPrefix + "/txs", {block: "@block"})
}).factory("TransactionsByAddress", function ($resource) {
    return $resource(window.apiPrefix + "/txs", {address: "@address"})
    ...
    return function (scope, element, attrs) {
        angular.element($window).bind("scroll", function () {
            200 <= this.pageYOffset ? scope.secondaryNavbar = !0 : scope.secondaryNavbar = !1, scope.$apply()
        })
    }
}).directive("whenScrolled", function ($window) {
    return {
        restric: "A", link: function (scope, elm, attr) {
    ...
    return {
        ...
        link: function (scope, elm) {
            elm.on("mousedown", function (event) {
                var text = scope.clipCopy, textArea = document.createElement("textarea");
                ...
                try {
                    ...
                    angular.element(elm[0].querySelector(".tooltip"))[0].style.display = "block", angular.element(elm[0].querySelector(".tooltip"))[0].style.opacity = "1", setTimeout(function () {
                        angular.element(elm[0].querySelector(".tooltip"))[0].style.opacity = "0", angular.element(elm[0].querySelector(".tooltip"))[0].style.display = "none"
                    }, 1e3)
                } catch (err) {
    ...
    $routeProvider.when("/block/:blockHash", {
        ...
        title: "Thought Transaction "
    }).when("/", {
        templateUrl: "views/index.html",
        title: "Home"
    }).when("/blocks", {
        templateUrl: "views/block_list.html",
        title: "Thought Blocks solved Today"
    }).when("/blocks-date/:blockDate/:startTimestamp?", {
        templateUrl: "views/block_list.html",
        title: "Dash Blocks solved "
    }).when("/address/:addrStr", {
        templateUrl: "views/address.html",
        title: "Thought Address "
    }).when("/status", {
    ...
    }).otherwise({templateUrl: "views/404.html", title: "Error"})
    ...
    gettextCatalog.currentLanguage = defaultLanguage, amMoment.changeLocale(defaultLanguage), $rootScope.$on("$routeChangeStart", function () {
        ...
        ngProgress.complete(), $rootScope.titleDetail = "", $rootScope.title = $route.current.title, $rootScope.currentAddr = null, $location.hash($routeParams.scrollTo), $anchorScroll()
    })
    ...
    gettextCatalog.setStrings("de_DE", {
        ...
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/dashpay/insight/issues" target="_blank">github issue tracker</a>.': '<strong>insight</strong> befindet sich aktuell noch in der Entwicklung. Bitte sende alle gefundenen Fehler (Bugs) und Feedback zur weiteren Verbesserung an unseren <a href="https://github.com/dashpay/insight-ui-dash/issues" target="_blank">Github Issue Tracker</a>.',
        About: "Über insight",
        Address: "Adresse",
        Age: "Alter",
        "Application Status": "Programmstatus",
        "Best Block": "Bester Block",
        "Dash node information": "Dash-Node Info",
        Block: "Block",
        "Block Reward": "Belohnung",
        Blocks: "Blöcke",
        "Bytes Serialized": "Serialisierte Bytes",
        "Can't connect to dashd to get live updates from the p2p network. (Tried connecting to dashd at {{host}}:{{port}} and failed.)": "Es ist nicht möglich mit dashd zu verbinden um live Aktualisierungen vom P2P Netzwerk zu erhalten. (Verbindungsversuch zu dashd an {{host}}:{{port}} ist fehlgeschlagen.)",
        ...
        "Last Block Hash (dashd)": "Letzter Hash (dashd)",
        ...
        '<strong>insight</strong>  is an <a href="http://live.insight.is/" target="_blank">open-source Dash blockchain explorer</a> with complete REST and websocket APIs that can be used for writing web wallets and other apps  that need more advanced blockchain queries than provided by dashd RPC.  Check out the <a href="https://github.com/dashpay/insight-ui-dash" target="_blank">source code</a>.': '<strong>insight</strong>  es un <a href="http://live.insight.is/" target="_blank">explorador de bloques de Dash open-source</a> con un completo conjunto de REST y APIs de websockets que pueden ser usadas para escribir monederos de Dashs y otras aplicaciones que requieran consultar un explorador de bloques.  Obtén el código en <a href="http://github.com/dashpay/insight" target="_blank">el repositorio abierto de Github</a>.',
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/dashpay/insight/issues" target="_blank">github issue tracker</a>.': '<strong>insight</strong> esta en desarrollo aún, por ello agradecemos que nos reporten errores o sugerencias para mejorar el software. <a href="https://github.com/dashpay/insight-ui-dash/issues" target="_blank">Github issue tracker</a>.',
        About: "Acerca de",
        Address: "Dirección",
        Age: "Edad",
        "Application Status": "Estado de la Aplicación",
        "Best Block": "Mejor Bloque",
        "Dash node information": "Información del nodo Dash",
        Block: "Bloque",
        "Block Reward": "Bloque Recompensa",
        Blocks: "Bloques",
        "Bytes Serialized": "Bytes Serializados",
        "Can't connect to dashd to get live updates from the p2p network. (Tried connecting to dashd at {{host}}:{{port}} and failed.)": "No se pudo conectar a dashd para obtener actualizaciones en vivo de la red p2p. (Se intentó conectar a dashd de {{host}}:{{port}} y falló.)",
        ...
        "Last Block Hash (dashd)": "Último Bloque Hash (dashd)",
        ...
        "(Input unconfirmed)": "(Entrée non confirmé)",
        "404 Page not found :(": "404 Page introuvable :(",
        '<strong>insight</strong>  is an <a href="http://live.insight.is/" target="_blank">open-source Dash blockchain explorer</a> with complete REST and websocket APIs that can be used for writing web wallets and other apps  that need more advanced blockchain queries than provided by dashd RPC.  Check out the <a href="https://github.com/dashpay/insight-ui-dash" target="_blank">source code</a>.': '<strong>insight</strong>  est un <a href="http://live.insight.is/" target="_blank">explorateur open-source de blocs</a> avec un ensemble complet d\'API REST et websockets qui peuvent être utilisées pour écrire portefeuille et autres applications nécéssitant des requêtes plus complète que celles proposés par les commandes RPC de dashd. Consultez le <a href="http://github.com/dashpay/insight" target="_blank">code source sur GitHub</a>.',
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/dashpay/insight/issues" target="_blank">github issue tracker</a>.': '<strong>insight</strong> est toujours en développement, hésitez pas à remonter tout bugs et proposer des améliorations sur notre <a href="https://github.com/dashpay/insight-ui-dash/issues" target="_blank">Github issue tracker</a>.',
        About: "À propos",
        Address: "Adresse",
        Age: "Âge",
        "Application Status": "Status de l'application",
        "Best Block": "Meilleur Bloc",
        "Dash node information": "Information sur le noeud Dash",
        Block: "Bloc",
        "Block Reward": "Recompense de bloc",
        Blocks: "Blocs",
        "Bytes Serialized": "Octets sérialisés",
        "Can't connect to dashd to get live updates from the p2p network. (Tried connecting to dashd at {{host}}:{{port}} and failed.)": "Impossible de se connecter à dashd pour récupérer les dernières informations du réseau p2p. (Tentative de connexion à dashd ({{host}}:{{port}}) échouée.)",
        "Can't connect to insight server. Attempting to reconnect...": "Impossible de se connecter au serveur Insight. Tentative de reconnexion...",
        "Can't connect to internet. Please, check your connection.": "Impossible de se connecter à internet. S'il vous plait, réessayer.",
        Complete: "Achevés",
        Confirmations: "Confirmations",
        Conn: "Conn",
        "Connections to other nodes": "Connexion aux autre noeuds",
        "Current Blockchain Tip (insight)": "Tip Actuel de la Blockchain (insight)",
        "Current Sync Status": "Status actuel de synchronisation",
        Details: "Détails",
        Difficulty: "Difficultée",
        "Double spent attempt detected. From tx:": "Tentative de double dépense détectée. Dans la tx:",
        "Error!": "Erreur!",
        Fee: "Frais",
        "Final Balance": "Solde final",
        "Finish Date": "Date de fin",
        "Go to home": "Retour à l'accueil",
        "Hash Serialized": "Hash Serialisé",
        Height: "Hauteur",
        "Included in Block": "Inclus dans le bloc",
        "Incoherence in levelDB detected:": "Incohérence dans levelDB détectée:",
        "Info Errors": "Erreurs d'informations",
        "Initial Block Chain Height": "Hauteur de la chaîne initiale",
        Input: "Entrée",
        "Last Block": "Dernier bloc",
        "Last Block Hash (dashd)": "Hash du dernier bloc  (dashd)",
        "Latest Blocks": "Derniers Blocs",
        "Latest Transactions": "Dernières transactions",
        "Loading Address Information": "Chargement des informations de l'adresse",
        "Loading Block Information": "Chagement des information du bloc",
        "Loading Selected Date...": "Chargement de la date sélectionnée...",
        "Loading Transaction Details": "Chargement des détails de la transaction",
        "Loading Transactions...": "Chagement des Transactions...",
        "Loading...": "Chargement...",
        "Mined Time": "Heure de minage",
        "Mined by": "Miné par",
        "Mining Difficulty": "Difficultée de minage",
        "Next Block": "Bloc suivant",
        "No Inputs (Newly Generated Coins)": "Pas d'entrées (nouvellement générées)",
        "No blocks yet.": "Aucun blocs pour l'instant.",
        "No matching records found!": "Aucun enregistremements correspondants trouvés!",
        "No. Transactions": "Transactions N°",
        "Number Of Transactions": "Nombre de transaction",
        Output: "Sortie",
        "Powered by": "Propulsé par",
        "Previous Block": "Bloc précédent",
        "Protocol version": "Version du protocol",
        "Proxy setting": "Paramètre de proxy",
        "Received Time": "Heure de réception",
        "Redirecting...": "Redirection...",
        "Search for block, transaction or address": "Rechercher des blocs, transactions ou adresses",
        "See all blocks": "Voir tous les blocs",
        "Show Transaction Output data": "Afficher les données de sortie de transaction",
        "Show all": "Tout afficher",
        "Show input": "Afficher l'entrée",
        "Show less": "Voir moins",
        "Show more": "Voir plus",
        Size: "Taille",
        "Size (bytes)": "Taille (octets)",
        "Skipped Blocks (previously synced)": "Blocs ignorés (préalablement synchronisés)",
        "Start Date": "Date de début",
        Status: "Status",
        Summary: "Résumé",
        "Summary <small>confirmed</small>": "Résume <small>confirmé</small>",
        "Sync Progress": "Progression de la synchronication",
        "Sync Status": "Status de la synchronisation",
        "Sync Type": "Type de la synchronisation",
        "Synced Blocks": "Blocs synchronisés",
        Testnet: "Réseau de test",
        "There are no transactions involving this address.": "Il n'y a pas de transactions impliquant cette adresse.",
        "Time Offset": "Décalage horaire",
        Timestamp: "Date et heure",
        Today: "Aujourd'hui",
        "Total Amount": "Montant Total",
        "Total Received": "Total Reçu",
        "Total Sent": "Total Envoyés",
        Transaction: "Transaction",
        "Transaction Output Set Information": "Informations sur l'ensemble des sorties de transaction",
        "Transaction Outputs": "Sorties de transaction",
        Transactions: "Transactions",
        Type: "Type",
        Unconfirmed: "Non confirmée",
        "Unconfirmed Transaction!": "Transaction non confirmée!",
        "Unconfirmed Txs Balance": "Solde non confirmé",
        "Value Out": "Valeur de sortie",
        Version: "Version",
        "Waiting for blocks...": "En attente de blocs...",
        "Waiting for transactions...": "En attente de transactions...",
        "by date.": "par date.",
        "first seen at": "aperçu la première fois le",
        mined: "miné",
        "mined on:": "miné le:"
        ...
        "Best Block": "最良ブロック",
        "Dash comes with a way of signing arbitrary messages.": "Dashには任意のメッセージを署名する昨日が備わっています。",
        "Dash node information": "Dashノード情報",
        Block: "ブロック",
        "Block Reward": "ブロック報酬",
        Blocks: "ブロック",
        "Broadcast Raw Transaction": "生のトランザクションを配信",
        "Bytes Serialized": "シリアライズ後の容量 (バイト)",
        "Can't connect to dashd to get live updates from the p2p network. (Tried connecting to dashd at {{host}}:{{port}} and failed.)": "P2Pネットワークからライブ情報を取得するためにdashdへ接続することができませんでした。({{host}}:{{port}} への接続を試みましたが、失敗しました。)",
        ...
        Input: "入力",
        "Last Block": "直前のブロック",
        "Last Block Hash (dashd)": "直前のブロックのハッシュ値 (dashd)",
        ...
        "This form can be used to broadcast a raw transaction in hex format over\n        the Dash network.": "このフォームでは、16進数フォーマットの生のトランザクションをDashネットワーク上に配信することができます。",
        "This form can be used to verify that a message comes from\n        a specific Dash address.": "このフォームでは、メッセージが特定のDashアドレスから来たかどうかを検証することができます。",
        "Time Offset": "時間オフセット",
        Timestamp: "タイムスタンプ",
        Today: "今日",
        "Total Amount": "Dash総量",
        ...
        "(Input unconfirmed)": "(Entrada não confirmada)",
        '<strong>insight</strong>  is an <a href="https://insight.is/" target="_blank">open-source Bitcoin blockchain explorer</a> with complete REST and websocket APIs that can be used for writing web wallets and other apps  that need more advanced blockchain queries than provided by bitcoind RPC.  Check out the <a href="https://github.com/bitpay/insight-ui" target="_blank">source code</a>.': '<strong>insight</Strong> é um <a href="https://insight.is/" target="_blank">explorador de blocos Bitcoin de código aberto</a> com um conjunto completo de APIs de websocket e REST que podem ser usadas para programar carteiras online e outros aplicativos que necessitam consultar um explorador de blocos. Confira o <a href="https://github.com/bitpay/insight-ui" target="_blank">código-fonte</a>.',
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/bitpay/insight-ui/issues" target="_blank">github issue tracker</a>.': '<Strong>insight</Strong> ainda está em desenvolvimento, portanto agradecemos que postem erros e sugestões para melhorias em nosso <a href="https://github.com/bitpay/insight-ui/issues" target="_blank">rastreador de problemas no GitHub</a>.',
        "404 Page not found :(": "404 Página não encontrada :(",
        About: "Sobre",
        Address: "Endereço",
        Age: "Idade",
        "An error occured in the verification process.": "Ocorreu um erro no processo de verificação.",
        "An error occured:<br>{{error}}": "Ocorreu um erro:<br>{{error}}",
        "Application Status": "Estado do Aplicativo",
        "Bitcoin comes with a way of signing arbitrary messages.": "Bitcoin possui um método de assinar mensagens arbitrárias.",
        "Bitcoin node information": "Informações do nó Bitcoin",
        Block: "Bloco",
        "Block Reward": "Recompensa do Bloco",
        Blocks: "Blocos",
        "Broadcast Raw Transaction": "Transmitir Transações Brutas",
        "by date.": "por data.",
        "Can't connect to bitcoind to get live updates from the p2p network. (Tried connecting to bitcoind at {{host}}:{{port}} and failed.)": "Não foi possível conectar-se ao bitcoind para obter atualizações em tempo real da rede p2p. (A tentativa de conectar-se à {{host}}:{{port}} falhou.)",
        "Can't connect to insight server. Attempting to reconnect...": "Não foi possível conectar-se ao servidor insight. Tentando reconectar...",
        "Can't connect to internet. Please, check your connection.": "Não foi possível conectar-se à internet. Verifique sua conexão.",
        Complete: "Completa",
        Confirmations: "Confirmações",
        Conn: "Con",
        "Connections to other nodes": "Conexões a outros nós",
        "Current Blockchain Tip (insight)": "Atual Blockchain Tip (insight)",
        "Current Sync Status": "Estado Atual de Sincronização",
        Details: "Detalhes",
        Difficulty: "Dificuldade",
        "Double spent attempt detected. From tx:": "Tentativa de gasto duplo detectada. Da transação:",
        "Error message:": "Mensagem de erro:",
        "Error!": "Erro!",
        Fee: "Taxa",
        "Fee Rate": "Taxa por kilobyte",
        "Final Balance": "Saldo Final",
        "Finish Date": "Data de Término",
        "first seen at": "visto pela primeira vez em",
        "Go to home": "Voltar ao início",
        Height: "Altura",
        "Included in Block": "Incluída no Bloco",
        "Incoherence in levelDB detected:": "Incoerência no levelDB detectada:",
        "Info Errors": "Erros de informação",
        "Initial Block Chain Height": "Altura inicial da Blockchain",
        Input: "Entrada",
        "Last Block": "Último Bloco",
        "Last Block Hash (Bitcoind)": "Hash do Último Bloco (Bitcoind)",
        "Latest Blocks": "Blocos Recentes",
        "Latest Transactions": "Transações Recentes",
        "Loading Address Information": "Carregando informações do endereço",
        "Loading Block Information": "Carregando informações do bloco",
        "Loading Selected Date...": "Carregando data selecionada...",
        "Loading Transaction Details": "Carregando detalhes da transação",
        "Loading Transactions...": "Carregando transações...",
        "Loading...": "Carregando...",
        Message: "Mensagem",
        mined: "minerado",
        "Mined by": "Minerado por",
        "mined on:": "minerado em:",
        "Mined Time": "Minerada em",
        "Mining Difficulty": "Dificuldade de Mineração",
        Network: "Rede",
        "Next Block": "Próximo Bloco",
        "No blocks yet.": "Ainda não há blocos.",
        "No Inputs (Newly Generated Coins)": "Nenhuma Entrada (moedas recém-geradas)",
        "No matching records found!": "Não foram encontrados registros correspondentes!",
        "No. Transactions": "Nº de transações",
        "Number Of Transactions": "Número de Transações",
        Output: "Saída",
        "Powered by": "Funciona com",
        "Previous Block": "Bloco Anterior",
        "Protocol version": "Versão do protocolo",
        "Proxy setting": "Configuração de proxy",
        "Raw transaction data": "Dados brutos da transação",
        "Raw transaction data must be a valid hexadecimal string.": "Os dados brutos da transação devem ser uma string hexadecimal válida.",
        "Received Time": "Recebida em",
        "Redirecting...": "Redirecionando...",
        "Search for block, transaction or address": "Procurar por blocos, transações ou endereços",
        "See all blocks": "Visualizar todos os blocos",
        ...
        "Skipped Blocks (previously synced)": "Blocos Ignorados (previamente sincronizados)",
        "Start Date": "Data de Início",
        Status: "Estado",
        Summary: "Resumo",
        "Summary <small>confirmed</small>": "Resumo <small>confirmado",
        "Sync Progress": "Progresso de Sincronização",
        "Sync Status": "Estado de Sincronização",
        "Sync Type": "Tipo de Sincronização",
        "Synced Blocks": "Blocos sincronizados",
        "The message failed to verify.": "Falha ao verificar a mensagem.",
        "The message is verifiably from {{verification.address}}.": "A mensagem é verificável de {{verification.address}}.",
        "There are no transactions involving this address.": "Não existem transações envolvendo este endereço.",
        ...
        "Time Offset": "Compensação de Tempo",
        Timestamp: "Data e Hora",
        Today: "Hoje",
        "Total Received": "Total Recebido",
        "Total Sent": "Total Enviado",
        Transaction: "Transação",
        "Transaction succesfully broadcast.<br>Transaction id: {{txid}}": "Transação transmitida com sucesso.<br>ID da transação: {{txid}}",
        Transactions: "Transações",
        Type: "Tipo",
        Unconfirmed: "Não confirmada",
        "Unconfirmed Transaction!": "Transação não confirmada!",
        "Unconfirmed Txs Balance": "Balanço de Transações não confirmadas",
        "Value Out": "Valor de Saída",
        Verify: "Verificar",
        "Verify signed message": "Verificar mensagem assinada",
        Version: "Versão",
        "Waiting for blocks...": "Esperando por blocos...",
        "Waiting for transactions...": "Esperando por transações..."
    }), gettextCatalog.setStrings("zh", {
        "(Input unconfirmed)": "输入未确定",
        ...
        '<strong>insight</strong> is still in development, so be sure to report any bugs and provide feedback for improvement at our <a href="https://github.com/bitpay/insight/issues" target="_blank">github issue tracker</a>.': '<strong>该系统</strong>仍在开发中<a href="https://github.com/bitpay/insight/issues" target="_blank">详情</a>发现问题或提供反馈请报告给我们的开发人员',
        About: "关于",
        Address: "地址",
        Age: "生成时间",
        "An error occured in the verification process.": "验证过程出错",
        "An error occured:<br>{{error}}": "发生错误<br>{{error}}",
        "Application Status": "程序状态",
        "Best Block": "最好的块",
        "Bitcoin comes with a way of signing arbitrary messages.": "Bitcoin可以签名任意消息",
        "Bitcoin node information": "比特币节点信息",
        Block: "区块",
        "Block Reward": "块报酬",
        Blocks: "区块",
        "Blocks Num": "区块数",
        "Broadcast Raw Transaction": "广播原始交易",
        "Bytes Serialized": "序列化后的大小 (B)",
        "Can't connect to bitcoind to get live updates from the p2p network. (Tried connecting to bitcoind at {{host}}:{{port}} and failed.)": "无法连接到BitCoin P2P网络。(尝试{{host}}:{{port}} 连接失败)",
        "Can't connect to insight server. Attempting to reconnect...": "无法连接到服务器，正在重试",
        "Can't connect to internet. Please, check your connection.": "无法连接到因特网，请检查连接",
        Complete: "完成",
        Confirmations: "个确认",
        Conn: "连接数",
        "Connections to other nodes": "连接数",
        "Current Blockchain Tip (insight)": "当前区块链Tip(insight)",
        "Current Sync Status": "当前同步状态",
        Details: "详情",
        Difficulty: "难度",
        "Double spent attempt detected. From tx:": "检测到双花，来自tx:",
        "Error message:": "错误消息:",
        "Error!": "错误",
        Fee: "手续费",
        "Final Balance": "钱包余额",
        "Finish Date": "结束日期",
        "Go to home": "回主页",
        "Hash Serialized": "Hash序列化",
        Height: "高度",
        BlockHeight: "区块高度",
        "Included in Block": "所在区块",
        "Incoherence in levelDB detected:": "在levelDB中检测到不一致：",
        "Info Errors": "错误信息",
        "Initial Block Chain Height": "最新区块高度",
        Input: "输入",
        "Last Block": "最新区块信息",
        "Last Block Hash (Bitcoind)": "最新区块哈希(Bitcoind)",
        "Latest Blocks": "最新区块",
        "Latest Transactions": "最新交易",
        "Loading Address Information": "加载地址信息",
        "Loading Block Information": "加载区块信息",
        "Loading Selected Date...": "加载选择日期...",
        "Loading Transaction Details": "加载交易详情",
        ...
        "Next Block": "下一个区块",
        "No Inputs (Newly Generated Coins)": "没有输入(新生产的币)",
        "No blocks yet.": "尚无区块",
        "No Transactions.": "尚无交易...",
        "No matching records found!": "没找到匹配的记录",
        "No. Transactions": "交易数量",
        "Number Of Transactions": "交易数目",
        Output: "输出",
        "Powered by": "Powered by",
        ...
        "Raw transaction data must be a valid hexadecimal string.": "原始交易数据必须是有效的十六进制",
        "Received Time": "接收时间",
        "Redirecting...": "重定向中...",
        "Search for block, transaction or address": "查询区块高度、区块哈希、交易哈希或地址",
        "See all blocks": "查看所有区块",
        "See all transcations": "查看所有交易",
        "Send transaction": "发送交易",
        "Show Transaction Output data": "显示交易输出数据",
        "Show all": "显示所有",
        "Show input": "显示输入",
        "Show less": "隐藏",
        "Show more": "显示更多",
        ...
        Size: "大小",
        "Size (bytes)": "大小(字节)",
        "Skipped Blocks (previously synced)": "已跳过块数量",
        "Start Date": "开始时间",
        Status: "状态",
        Summary: "摘要",
        "Summary <small>confirmed</small>": "摘要<small>已确认</small>",
        "Sync Progress": "同步进度",
        "Sync Status": "同步状态",
        "Sync Type": "同步类型",
        "Synced Blocks": "已同步块数量",
        Testnet: "Testnet",
        "The message failed to verify.": "验证消息失败",
        "The message is verifiably from {{verification.address}}.": "消息由{{verification.address}}验证",
        "There are no transactions involving this address.": "此地址没有发生过交易",
        "This form can be used to broadcast a raw transaction in hex format over\n        the Bitcoin network.": "此表单可用于以十六进制格式通过比特币网络广播一个原始事务。",
        "This form can be used to verify that a message comes from\n        a specific Bitcoin address.": "此表单可用于验证消息是否来自特定的比特币地址。",
        "Time Offset": "时间偏移",
        Timestamp: "交易时间",
        Today: "今天",
        "Total Amount": "总数",
        ...
        "Transaction Output Set Information": "交易输出集合信息",
        "Transaction Outputs": "交易输出",
        "Transaction succesfully broadcast.<br>Transaction id: {{txid}}": "交易成功播报，<br>交易ID: {{txid}}",
        Transactions: "交易",
        Type: "类别",
        Unconfirmed: "未确认",
        "Unconfirmed Transaction!": "未确认!",
        "Unconfirmed Txs Balance": "未确认的交易余额",
        "Value Out": "输出值",
        Verify: "验证",
        "Verify signed message": "验证签名消息",
        Version: "版本",
        "Waiting for blocks...": "等待区块...",
        "Waiting for transactions...": "等待交易...",
        "by date.": "按日期",
        "first seen at": "最初发现于",
        mined: "挖掘",
        "mined on:": "挖掘日期",
